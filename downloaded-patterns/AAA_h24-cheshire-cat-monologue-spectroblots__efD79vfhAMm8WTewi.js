// Pattern: AAA h24-cheshire-cat-monologue-spectroblots
// ID: efD79vfhAMm8WTewi
...
//
// TODOs:
// - Replicate the active sound pattern across the 3 segments instead of across whole set of pixels
//   I.E. treat the cat as the entire voice canvas and replicate same pulses in shrooms
//   ...because otherwise it ends up looking a bit fragmented
//
// - Implement a smoother transition between idle and active
// - Add brightness scale factors for both idle and speak

/*
  Based on public pattern: Sound - spectroblots 1D/2D/3D
*/

// Only true if kitty dialog is currently active
export var aaaKittyActive = false;

// Displaying the boolean active state as a gauge just because it's a bit easier to read at a glance.
export function gaugeKittyActive() {
  return aaaKittyActive ? 1 : 0;
}

// Render the idle sequence.
export var idleSequenceTime = -1;
function renderIdleHueCycler(index) {
  // TODO Make this iterate over mushrooms only... or just explicitly black out the cat?
  if (index < 37) {
    hsv(0, 0, 0);
    return;
  }

  h = idleSequenceTime + index / pixelCount;
  s = 1;
  v = 1;
  hsv(h, s, v);
}
...
export function beforeRender(delta) {
  // Update active state based on sensor board's running sound average.
  // i.e. if there's any sound coming through above baseline noise, light up the kitty.
  aaaKittyActive = energyAverage > 0.001;
...
export function render(index, x) {
  if (aaaKittyActive) render3D(index, x, 0, 0);
  else renderIdleHueCycler(index);
}
