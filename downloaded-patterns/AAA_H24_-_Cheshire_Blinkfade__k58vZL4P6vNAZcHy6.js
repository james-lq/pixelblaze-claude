// Pattern: AAA H24 - Cheshire Blinkfade
// ID: k58vZL4P6vNAZcHy6
...
*/
pic = makePIController(0.05, 0.15, 300, 0, 1000);
...
export function beforeRender(delta) {
  ...
  if (energyAverage == -1) {
    // No sensor board is connected
    simulateSound();
  } else {
    // Load the live data from the sensor board
  ...
  for (i = 0; i < pixelCount; i++) {
    ...
    vals[i] -= 0.0005 * delta + abs((_energyAverage * sensitivity) / 5000);
    ...
    if (vals[i] <= 0) {
      ...
      */
      hues[i] = time(0.07) + 0.2 * triangle(_maxFrequency / 1000);
    }
  }
}
...
var _energyAverage = 0,
  _maxFrequency = 0,
  _maxFrequencyMagnitude = 0;
...
function simulateFrequencyData(fBin) {
  ...
  for (inst = 0; inst < instrumentCount; inst++) {
    ...
    if ((instruments[inst][0] >> (15 - sequencerSlot)) & 1) {
      ...
      binProximity = max(
        0,
        1 - abs(fBin - instruments[inst][1]) / instruments[inst][2],
      );
      ...
      fDataBinSum +=
        binomSample(instruments[inst][3], 3) * slotProximity * binProximity;
    }
  }
