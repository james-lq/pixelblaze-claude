// Pattern: AAA KFLiP26 - sound - spectrum analyser 1D/2D
...
averageMax = 0;
pic = makePIController(0.25, 1.8, 30, 0, 100);
...
function simulateSound() {
  ...
  claps = 0.01 * square(2 * tM - 0.5, 0.1); // "&" of every beat
  for (i = 9; i < 14 + random(10); i++)
    frequencyData[i] += claps * (0.7 + 0.6 * random(i % 2));

  highHat = 0.003 * square(4 * tM - 0.5, 0.05); // Beats 2 and 4
  for (i = 20; i < 30; i++) {
    frequencyData[i] += highHat * (0.8 + random(0.4)) * (i % 3 < 2);
  }

  lead = 4 + floor(16 * wander(tP)); // Wandering fundamental synth's freq bin
  for (i = 4; i < 20; i++)
    // Excite the fundamental and, 20% of the time, 4 bins up
    frequencyData[i] += 0.005 * (lead == i || lead == (i - 4) * r(0.2));
}
