...
Prototyping rig: the Pixelblaze registered as **`0x00AC056C`** in the workspace `devices.toml`, whose `notes` field carries the device-level facts (board, firmware, channel wiring). Address and firmware live there, not here, so they stay correct when either changes.
...
**No pixel map for this project** — write `render(index)` only, never `render2D`. (The device had been carrying the `sound-level-meter` mapper, an 8x12 / 100-pixel grid that does not describe this rig; it was cleared by hand through the web UI on 2026-09-07.)
